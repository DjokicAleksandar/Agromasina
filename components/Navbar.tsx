...
const Navbar = () => {
  ...
  return (
    <>
      <nav
        className={`fixed transition-all duration-300 z-20 ${
          isVisible ? "translate-y-0" : "-translate-y-full"
        } 
                flex w-full justify-between absolute ${
                  isMobile ? "p-3 pt-2 pb-2" : "p-6 pt-4 pb-4"
                } ${isDown ? "shadow-md bg-white" : "bg-transparent"}`}
      >
        <Image
          src={
            !isMobile
          ...
          <Link href={"/contact"}>
            <FaPhoneAlt
              className={`${isMobile ? "text-2xl" : "text-3xl"} ${
                !isMobile ? (isDown ? "text-black" : "text-black") : ""
